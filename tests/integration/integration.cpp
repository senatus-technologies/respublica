...
TEST_F( integration, token )
{
  ...
  koinos::protocol::transaction transaction = make_transaction(
    alice_secret_key,
    1,
    8'000'000,
...
TEST_F( integration, coin )
{
  ...
  response = _controller->read_program(
    coin,
    make_arguments( test::token_entry::balance_of, koinos::protocol::user_account( alice_secret_key.public_key() ) ) );
  ...
  response = _controller->read_program(
    coin,
    make_arguments( test::token_entry::balance_of, koinos::protocol::user_account( bob_secret_key.public_key() ) ) );
