...
struct is_container<
  T,
  std::void_t<
    decltype( std::begin( std::declval< T >() ), std::end( std::declval< T >() ), std::size( std::declval< T >() ) ) > >
    : std::true_type
{};
...
as_impl( Container& c, const T& t, std::size_t start )
{
  ...
  std::transform( std::begin( t ),
                  std::end( t ) - ( std::size( t ) - items_to_add ),
                  std::begin( c ) + start,
                  []( auto ch )
                  {
...
typename std::enable_if_t< is_container< T >::value, void > to_impl( std::stringstream& ss, T& t )
{
  ...
  std::transform( std::begin( s ) + pos,
                  std::end( s ),
                  std::begin( t ),
                  []( auto ch )
                  {
                    return reinterpret_cast< decltype( *std::begin( t ) ) >( ch );
                  } );
}
...
std::enable_if_t< detail::greater_than< sizeof...( Ts ), 1 >::value, Container > as( Ts... ts )
{
  Container c;
  static_assert( sizeof( *c.begin() ) == sizeof( std::byte ) );
...
std::enable_if_t< detail::greater_than< sizeof...( Ts ), 1 >::value, std::tuple< Ts... > > to( const Container& c )
{
  static_assert( sizeof( *std::begin( c ) ) == sizeof( std::byte ) );
  std::stringstream ss;
