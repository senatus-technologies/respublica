...
extern int32_t koinos_get_object( uint32_t id, const char* key_ptr, uint32_t key_len, char* ret_ptr, uint32_t* ret_len );
extern int32_t koinos_put_object( uint32_t id, const char* key_ptr, uint32_t key_len, const char* value_ptr, uint32_t value_len );
extern int32_t koinos_check_authority( const char* account_ptr, uint32_t account_len, const char* data_ptr, uint32_t data_len, bool* value );
...
char* get_caller()
{
  ...
  int32_t ret_val = koinos_get_caller(
    syscall_buffer,
    &ret_len );
...
size_t get_object( uint32_t id, const char* key, size_t key_size, char** value )
{
  ...
  int32_t ret_val = koinos_get_object(
    id,
    key,
    key_size,
    syscall_buffer,
...
{
  int32_t ret_val = koinos_put_object(
    id,
    key,
    key_size,
    value,
...
bool check_authority( const char* address )
{
  ...
  int32_t ret_val = koinos_check_authority(
    address,
    address_size,
    empty_string,
    0,
