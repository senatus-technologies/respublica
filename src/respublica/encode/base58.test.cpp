...
TEST( base58, encode )
{
  auto encoded_data = respublica::encode::to_base58( respublica::memory::as_bytes( str ) );
...
TEST( base58, decode )
{
  auto decoded_data = respublica::encode::from_base58( b58_str );

  EXPECT_TRUE( decoded_data );
  EXPECT_TRUE( std::ranges::equal( *decoded_data, std::as_bytes( std::span( str ) ) ) );

  decoded_data = respublica::encode::from_base58( "0DdiPPYtxLjCD3wA1po2rvZHTDYjkZYiEtazrfiwJcwnKCizhGFhBGHeRdx"sv );
  if( decoded_data )
