...
frame_message( message_type_id type_id, std::uint16_t version, std::vector< std::byte > payload )
{
  ...
  message_frame frame;
  frame.payload = std::move( payload ); // Move payload (zero-copy)
  ...
  std::memcpy( frame.header.data() + offset, &length_le, sizeof( std::uint32_t ) );
  ...
  std::memcpy( frame.header.data() + offset, &type_id_le, sizeof( std::uint32_t ) );
  offset += sizeof( std::uint32_t );
  ...
  std::memcpy( frame.header.data() + offset, &version_le, sizeof( std::uint16_t ) );
