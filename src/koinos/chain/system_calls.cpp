...
void register_thunks( thunk_dispatcher& td )
{
  THUNK_REGISTER_GENESIS(
    ...
    (get_head_info)(apply_block)(apply_transaction)(apply_upload_contract_operation)(apply_call_contract_operation)(apply_set_system_call_operation)(apply_set_system_contract_operation)(pre_block_callback)(post_block_callback)(pre_transaction_callback)(post_transaction_callback)( get_chain_id )

    // System Helpers
    (process_block_signature)(get_transaction)(get_transaction_field)(get_block)(get_block_field)(get_last_irreversible_block)(get_account_nonce)(verify_account_nonce)(set_account_nonce)(check_system_authority)( get_operation )

    // Resource Subsystem
    (get_account_rc)(consume_account_rc)(get_resource_limits)( consume_block_resources )

    // Database
    (put_object)(remove_object)(get_object)(get_next_object)( get_prev_object )

    // Logging
    (log)( event )

    // Cryptography
    (hash)(recover_public_key)(verify_merkle_root)(verify_signature)( verify_vrf_proof )

    // Contract Management
    (call)(exit)(get_arguments)(get_contract_id)(get_caller)( check_authority ) )
...
THUNK_DEFINE( void, apply_block, ( (const protocol::block&)block ) )
{
  ...
  try
  {
    ...
    KOINOS_ASSERT( system_call::hash( context,
    ...
    const crypto::multihash tx_root =
      util::converter::to< crypto::multihash >( block.header().transaction_merkle_root() );
    KOINOS_ASSERT( tx_root.code() == context.block_hash_code(),
                   malformed_block_exception,
                   "unexpected transaction merkle root hash code - was: ${t}, expected: ${e}",
                   ( "t", std::underlying_type_t< crypto::multicodec >( tx_root.code() ) )(
                     "e",
                     std::underlying_type_t< crypto::multicodec >( context.block_hash_code() ) ) );
    ...
    for( const auto& trx: block.transactions() )
    {
      ...
      hashes.emplace_back( system_call::hash( context,
                                              std::underlying_type_t< crypto::multicodec >( context.block_hash_code() ),
                                              util::converter::as< std::string >( trx.header() ) ) );
      std::stringstream ss;
    ...
    KOINOS_ASSERT( system_call::verify_merkle_root( context, block.header().transaction_merkle_root(), hashes ),
                   malformed_block_exception,
    ...
    auto block_hash = util::converter::to< crypto::multihash >(
      system_call::hash( context,
                         std::underlying_type_t< crypto::multicodec >( context.block_hash_code() ),
                         util::converter::as< std::string >( block.header() ) ) );
    KOINOS_ASSERT( system_call::process_block_signature( context,
    ...
    uint64_t system_rc_cost = meter.system_disk_storage_used() * rld.disk_storage_cost()
    ...
    KOINOS_ASSERT( system_call::consume_account_rc( context, block.header().signer(), system_rc_cost ),
                   insufficient_rc_exception,
                   "unable to consume system rc for block producer: ${p}",
                   ( "p", util::to_base58( block.header().signer() ) ) );
    ...
    KOINOS_ASSERT( system_call::consume_block_resources( context,
...
THUNK_DEFINE( void, apply_transaction, ( (const protocol::transaction&)trx ) )
{
  ...
  try
  {
    ...
    try
    {
      ...
      KOINOS_ASSERT( payer_rc >= trx.header().rc_limit(),
                     failure_exception,
                     "payer does not have the rc to cover transaction rc limit" );
      ...
      const crypto::multihash op_root =
        util::converter::to< crypto::multihash >( trx.header().operation_merkle_root() );
      KOINOS_ASSERT( op_root.code() == context.block_hash_code(),
                     failure_exception,
                     "unexpected operation merkle root hash code - was: ${o}, expected: ${e}",
                     ( "o", std::underlying_type_t< crypto::multicodec >( op_root.code() ) )(
                       "e",
                       std::underlying_type_t< crypto::multicodec >( context.block_hash_code() ) ) );
      ...
      for( const auto& op: trx.operations() )
        hashes.emplace_back(
          system_call::hash( context,
                             std::underlying_type_t< crypto::multicodec >( context.block_hash_code() ),
                             util::converter::as< std::string >( op ) ) );

      KOINOS_ASSERT( system_call::verify_merkle_root( context, trx.header().operation_merkle_root(), hashes ),
                     failure_exception,
                     "operation merkle root does not match" );

      auto authorized = system_call::check_authority( context, transaction_application, payer );
      KOINOS_ASSERT( authorized,
                     authorization_failure_exception,
                     "account ${account} has not authorized transaction",
                     ( "account", util::to_base58( payer ) ) );
      ...
      if( use_payee_nonce )
      {
        ...
        KOINOS_ASSERT( authorized,
                       authorization_failure_exception,
                       "account ${account} has not authorized transaction",
                       ( "account", util::to_base58( payee ) ) );
      }

      KOINOS_ASSERT( system_call::verify_account_nonce( context, nonce_account, trx.header().nonce() ),
    ...
    KOINOS_ASSERT( system_call::consume_account_rc( context, payer, used_rc ),
                   failure_exception,
                   "unable to consume rc for payer: ${p}",
                   ( "p", util::to_base58( payer ) ); );
...
THUNK_DEFINE( void, apply_upload_contract_operation, ( (const protocol::upload_contract_operation&)o ) )
{
  ...
  KOINOS_ASSERT( authorized,
                 authorization_failure_exception,
                 "account ${account} has not authorized action",
                 ( "account", util::to_base58( o.contract_id() ) ) );
  ...
  contract_meta.set_hash( system_call::hash( context,
                                             std::underlying_type_t< crypto::multicodec >( context.block_hash_code() ),
                                             o.bytecode() ) );
  ...
  system_call::put_object( context,
                           state::space::contract_metadata(),
                           o.contract_id(),
                           util::converter::as< std::string >( contract_meta ) );
}
...
THUNK_DEFINE( void, apply_set_system_call_operation, ( (const protocol::set_system_call_operation&)o ) )
{
  ...
  KOINOS_ASSERT( !context.read_only(),
                 read_only_context_exception,
                 "unable to perform action while context is read only" );
  KOINOS_ASSERT( system_call::check_system_authority( context ),
                 system_authorization_failure_exception,
                 "system authority required" );

  if( o.target().has_system_call_bundle() )
  {
    auto contract_object = system_call::get_object( context,
                                                    state::space::contract_bytecode(),
                                                    o.target().system_call_bundle().contract_id() );
    KOINOS_ASSERT( contract_object.exists(), invalid_contract_exception, "contract does not exist" );
    auto contract_meta_object = system_call::get_object( context,
                                                         state::space::contract_metadata(),
                                                         o.target().system_call_bundle().contract_id() );
  ...
  {
    KOINOS_ASSERT( thunk_dispatcher::instance().thunk_exists( o.target().thunk_id() ),
  ...
  system_call::put_object(
    context,
    state::space::system_call_dispatch(),
    util::converter::as< std::string >( std::underlying_type_t< koinos::chain::system_call_id >( o.call_id() ) ),
    util::converter::as< std::string >( o.target() ) );
...
THUNK_DEFINE( process_block_signature_result,
              process_block_signature,
              ( (const std::string&)id, (const protocol::block_header&)header, (const std::string&)signature_data ) )
{
  ...
  ret.set_value( genesis_addr
                 == util::converter::to< crypto::public_key >(
                      system_call::recover_public_key( context, ecdsa_secp256k1, signature_data, id, true ) )
                      .to_address_bytes() );
  return ret;
}
...
THUNK_DEFINE( verify_account_nonce_result,
              verify_account_nonce,
              ( (const std::string&)account, (const std::string&)nonce ) )
{
  auto nonce_value = util::converter::to< value_type >( nonce );
  KOINOS_ASSERT( nonce_value.has_uint64_value(),
                 invalid_nonce_exception,
  ...
  KOINOS_ASSERT( current_nonce_value.has_uint64_value(),
                 invalid_nonce_exception,
...
THUNK_DEFINE_VOID( check_system_authority_result, check_system_authority )
{
  ...
  for( const auto& sig: trx->signatures() )
  {
    auto addr = util::converter::to< crypto::public_key >(
                  system_call::recover_public_key( context, ecdsa_secp256k1, sig, trx->id(), true ) )
                  .to_address_bytes();
...
THUNK_DEFINE( consume_block_resources_result,
              consume_block_resources,
              ( (uint64_t)disk, (uint64_t)network, (uint64_t)compute ) )
{
...
THUNK_DEFINE( void,
              event,
              ( (const std::string&)name, (const std::string&)data, (const std::vector< std::string >&)impacted ) )
{
  ...
  context.resource_meter().use_compute_bandwidth( context.get_compute_bandwidth( "event_per_impacted" )
                                                  * impacted.size() );
...
THUNK_DEFINE( hash_result, hash, ( (uint64_t)id, (const std::string&)obj, (uint64_t)size ) )
{
  ...
  context.resource_meter().use_compute_bandwidth( context.get_compute_bandwidth( hash_base )
                                                  + context.get_compute_bandwidth( hash_per_byte ) * obj.size() );
...
THUNK_DEFINE( recover_public_key_result,
              recover_public_key,
              ( (dsa)type, (const std::string&)signature_data, (const std::string&)digest, (bool)compressed ) )
{
  ...
  KOINOS_ASSERT( crypto::public_key::is_canonical( signature ),
                 invalid_signature_exception,
...
THUNK_DEFINE( verify_merkle_root_result,
              verify_merkle_root,
              ( (const std::string&)root, (const std::vector< std::string >&)hashes ) )
{
  ...
  context.resource_meter().use_compute_bandwidth(
    ( hashes.size() + 1 ) * ( deserialize_multihash_base + root.size() * deserialize_multihash_per_byte ) );
  ...
  context.resource_meter().use_compute_bandwidth( hashes_per_leaves( hashes.size() )
                                                  * ( hash_base + 2 * root_hash.digest().size() * hash_per_byte ) );
  ...
  std::transform( std::begin( hashes ),
                  std::end( hashes ),
                  std::begin( leaves ),
                  [ & ]( const std::string& s )
                  {
                    auto mh = util::converter::to< crypto::multihash >( s );
                    KOINOS_ASSERT( mh.code() == root_hash.code(),
                                   unknown_hash_code_exception,
                                   "leaf and merkle root hash codes do not match" );
                    KOINOS_ASSERT( mh.digest().size() == root_hash.digest().size(),
                                   unknown_hash_code_exception,
                                   "leaf and merkle root hash sizes do not match" );
                    return mh;
                  } );
...
THUNK_DEFINE( verify_signature_result,
              verify_signature,
              ( (dsa)type,
...
THUNK_DEFINE( verify_vrf_proof_result,
              verify_vrf_proof,
              ( (dsa)type,
                (const std::string&)public_key,
                ...
                (const std::string&)message ) )
{
  ...
  catch( const crypto::vrf_validation_error& )
  { /* do nothing */
  }
...
THUNK_DEFINE( call_result, call, ( (const std::string&)contract_id, (uint32_t)entry_point, (const std::string&)args ) )
{
  ...
  KOINOS_ASSERT( entry_point != authorize_entrypoint || context.get_caller_privilege() == privilege::kernel_mode,
                 insufficient_privileges_exception,
                 "calling privileged thunk from non-privileged code" );
  ...
  try
  {
    with_stack_frame(
      context,
      stack_frame{ .contract_id    = contract_id,
              ...
              ( (authorization_type)x, (const std::string&)account, (const std::string&)data ) )
{
  ...
  if( authorize_override )
  {
    ...
    authorized =
      util::converter::to< authorize_result >(
        system_call::call( context, account, authorize_entrypoint, util::converter::as< std::string >( args ) ) )
        .value();
  }
  else
  {
