...
int32_t host_api::koinos_get_caller( char* ret_ptr, uint32_t* ret_len )
{
  if( auto caller = _ctx.get_caller(); caller )
  {
    ...
    std::copy( caller->begin(), caller->end(), std::as_writable_bytes( std::span( ret_ptr, *ret_len ) ).begin() );
    *ret_len = caller->size();
  }
  else
...
host_api::koinos_get_object( uint32_t id, const char* key_ptr, uint32_t key_len, char* ret_ptr, uint32_t* ret_len )
{
  if( auto object = _ctx.get_object( id, std::as_bytes( std::span( key_ptr, key_len ) ) ); object )
  {
    ...
    std::copy( object->begin(), object->end(), std::as_writable_bytes( std::span( ret_ptr, *ret_len ) ).begin() );
    *ret_len = object->size();
  }
  else
                                          ...
                                          bool* value )
{
  ...
  auto account_span = std::as_bytes( std::span( account_ptr, account_len ) );
  std::copy( account_span.begin(), account_span.end(), account.end() );
