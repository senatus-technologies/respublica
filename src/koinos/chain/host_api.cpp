  ...
  with_stack_frame(
    ...
    [ & ]()
    {
      ...
      else
      {
        ...
        auto compute = _ctx.get_compute_bandwidth( std::string( enum_value->name() ) );
...
uint32_t host_api::wasi_fd_write( uint32_t fd, const uint8_t* iovs, uint32_t iovs_len, uint32_t* nwritten )
{
  ...
  return with_stack_frame( _ctx,
                           ...
                           [ & ]
                           {
                             ...
                             _ctx.write_output(
                               std::span< const std::byte >( reinterpret_cast< const std::byte* >( iovs ), iovs_len ) );
                             *nwritten = iovs_len;
