...
struct system_interface
{
  ...
  virtual std::expected< bytes_v, error >
  call_program( const protocol::account& account, uint32_t entry_point, const std::vector< bytes_s >& args ) = 0;
};
...
// std::expected< bytes_v, error_code > recover_public_key( uint64_t dsa, bytes_s signature, bytes_s digest, bool
// compressed );
