...
public_key_view::public_key_view( public_key_span pks ) noexcept:
  _bytes( pks )
{
#ifndef FAST_CRYPTO
  initialize_crypto();
...
bool public_key_view::operator==( const public_key_view& rhs ) const noexcept
{
...
bool public_key_view::verify( const signature& sig, const digest& dig ) const noexcept
{
#ifdef FAST_CRYPTO
...
public_key_span public_key_view::bytes() const noexcept
{
  return _bytes;
}
