...
TEST( multihash, zero )
{
  auto mh = koinos::crypto::multihash::zero( koinos::crypto::multicodec::sha2_256 );
  if( !mh )
    FAIL();
  else
  {
...
{
  auto mh = koinos::crypto::multihash::empty( koinos::crypto::multicodec::sha2_256 );
  ASSERT_TRUE( mh );
