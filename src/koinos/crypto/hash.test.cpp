...
TEST( hash, blake3 )
{
  ...
  EXPECT_TRUE(
    std::ranges::equal( out2, koinos::util::from_base58( "4rfJRE9JwBL4vMJMUntTcDF5TRDEmqfU5tfiQFBzt7nV" ) ) );
  ...
  EXPECT_TRUE(
    std::ranges::equal( out3, koinos::util::from_base58( "9naWkD2RN6dS65KiTDogGSPcrDJaBywdEdD1zwrhHxKs" ) ) );

  std::uint64_t number = 12'345;
  auto out4            = koinos::crypto::hash( number );
  EXPECT_TRUE(
    std::ranges::equal( out4, koinos::util::from_base58( "HnXg8eU4ELYHMkXCtDRM1paFQnMAGgxrhehAkEgopFh3" ) ) );
  ...
  EXPECT_TRUE(
    std::ranges::equal( out5, koinos::util::from_base58( "9naWkD2RN6dS65KiTDogGSPcrDJaBywdEdD1zwrhHxKs" ) ) );
}
