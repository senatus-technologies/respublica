...
TEST( elliptic, ecc )
{
  ...
  for( uint32_t i = 0; i < 100; ++i )
  {
    ...
    pass                              += "1";
    koinos::crypto::multihash h2       = hash( koinos::crypto::multicodec::sha2_256, pass.c_str(), pass.size() );
...
TEST( elliptic, wif )
{
  ...
  koinos::crypto::private_key key1 = koinos::crypto::private_key::regenerate(
  ...
  EXPECT_THROW( koinos::crypto::private_key::from_wif( wif ), koinos::crypto::key_serialization_error );
...
TEST( elliptic, address )
{
  ...
  EXPECT_EQ( address, address_bytes );
}
...
TEST( elliptic, vrf )
{
  ...
  for( int i = 0; i < proof.size(); i++ )
  {
    ...
    EXPECT_THROW( pub_key.verify_random_proof( msg, proof ), koinos::crypto::vrf_validation_error );
  ...
  EXPECT_EQ( koinos::util::to_hex( proof_hash.digest() ), expected_hash );
