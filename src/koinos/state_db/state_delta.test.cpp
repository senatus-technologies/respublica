...
{
  auto delta = std::make_shared< koinos::state_db::state_delta >();
  ASSERT_TRUE( delta );
  ...
  EXPECT_EQ( delta->id(), koinos::state_db::state_node_id{} );
  ...
  if( auto clone = delta->clone(); clone )
  {
    ...
    EXPECT_EQ( clone->id(), koinos::state_db::state_node_id{} );
...
{
  auto parent = std::make_shared< koinos::state_db::state_delta >();
  ASSERT_TRUE( parent );
  ...
  EXPECT_EQ( child->id(), koinos::state_db::digest{ std::byte{ 0x01 } } );
  EXPECT_EQ( child->parent_id(), koinos::state_db::digest{} );
...
TEST( state_delta, commit )
{
  ...
  auto root = std::make_shared< koinos::state_db::state_delta >();
...
TEST( state_delta, merkle_root )
{
  ...
  EXPECT_TRUE( std::ranges::equal( delta->merkle_root(), koinos::crypto::merkle_root( merkle_leafs ) ) );
