...
public:
  ...
  virtual std::int32_t
  wasi_fd_write( std::uint32_t fd, const std::uint8_t* iovs, std::uint32_t iovs_len, std::uint32_t* nwritten ) = 0;
  virtual std::int32_t wasi_fd_close( std::uint32_t fd )                                                       = 0;
  virtual std::int32_t wasi_fd_fdstat_get( std::uint32_t fd, std::uint8_t* buf_ptr )                           = 0;
