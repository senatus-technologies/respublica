...
using public_key_data_view = std::span< const std::byte, public_key_length >;
...
public:
  public_key() = delete;
  public_key( public_key_data_view pks ) noexcept;
  ...
  public_key_data_view bytes() const noexcept;

private:
  public_key_data_view _bytes;
};
